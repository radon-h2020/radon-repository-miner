...
from repominer.files import FixingFile, FixingFileEncoder


class CLIMineFixingFilesTestCase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.exclude_fixing_files_file = os.path.join(cls.path_to_tmp_dir, 'exclude_files.json')
        ...
        with open(cls.exclude_fixing_files_file, 'w') as f:
    ...
    def test_mine_fixing_files(self):
        result = os.system(
            'repo-miner mine fixing-files github tosca UoW-CPC/COLARepo {}'.format(self.path_to_tmp_dir))

        assert result == 0

        with open(os.path.join(self.path_to_tmp_dir, 'fixed-files.json'), 'r') as f:
            fixing_files = json.load(f)
            assert len(fixing_files) == 8

    def test_mine_fixing_files_with_exclude(self):
        result = os.system(
            'repo-miner mine fixing-files github tosca UoW-CPC/COLARepo {0} --exclude-files {1}'.format(
                self.path_to_tmp_dir,
                self.exclude_fixing_files_file))
